...
export default function OverviewPage() {
  ...
  // Subscribe to SSE events via SseProvider
      ...
      switch (type) {
        case "TOPUP_CREATED":
          setApiData((prev) => {
            if (!prev) return prev;
            const next = [...prev];
            next[0] = { ...next[0], remittances: String(Number(next[0].remittances || "0") + 1) };
            return next;
          });
          break;
        ...
        case "SALE_CREATED":
          setApiData((prev) => {
            if (!prev) return prev;
            const next = [...prev];
            next[0] = { ...next[0], salesReport: String(Number(next[0].salesReport || "0") + 1) };
            return next;
          });
          break;
        ...
        case "REMITTANCE_CREATED":
          setApiData((prev) => {
            if (!prev) return prev;
            const next = [...prev];
            next[0] = { ...next[0], actual_remittances: String(Number(next[0].actual_remittances || "0") + 1) };
            return next;
          });
          break;
        ...
        case "SHORTAGE_CREATED":
          setApiData((prev) => {
            if (!prev) return prev;
            const next = [...prev];
            next[0] = { ...next[0], varianceSum: String(Number(next[0].varianceSum || "0") + 1000) };
            return next;
          });
          break;
      }
