...
import { reverseTopUp } from "@/app/server/services/topup.service";
import { reverseTopUpSchema } from "@/app/schemas/auth.schema";
...
export async function POST(req: Request) {
    try {
        ...
        if (!session?.user || !session.user.id || (session.user.role !== "ADMIN"&&session.user.role !== "AUDITOR")) {
