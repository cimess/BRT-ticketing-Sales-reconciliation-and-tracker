...
export async function checkAndEscalateExpectations(companyId: string) {
 ...
 // 1. Include user, device, sales reports, and locations to construct human-readable descriptions
   ...
   if (targetStatus && targetStatus !== exp.status) {
      const fineResult = await prisma.$transaction(async (tx) => {
        ...
        if (targetStatus === "VIOLATED") {
          const ticketerName = exp.pos_session?.user
            ? `${exp.pos_session.user.first_name} ${exp.pos_session.user.last_name}`
            : `Ticketer`;
          const deviceName = exp.pos_session?.device?.name || "POS Device";
          const fineReason = `Overdue Shortage Remittance: ${ticketerName} on ${deviceName}`;
      ...
      if (fineResult?.fineCreated) {
        ...
        await sendNotification({
          companyId: companyId,
          message: `System issued a late remittance fine of ₦${formattedAmount} to ${ticketerName} on ${deviceName}.`,
          type: "FINE_ISSUED",
          referenceId: fine.id,
          target: {
            userIds: [exp.user_id],
  ...
  for (const remit of pendingDeposits) {
    ...
    if (now > deadline) {
      ...
      const fineResult = await prisma.$transaction(async (tx) => {
        ...
        const ticketerName = remit.ticketer 
          ? `${remit.ticketer.first_name} ${remit.ticketer.last_name}`
          : "Ticketer";
        const fineReason = `Late bank deposit violation for remittance from ${ticketerName}`;
        ...
        const fine = fineResult.fineCreated;
        const formattedAmount = fine.amount ? Number(fine.amount).toLocaleString() : "TBD";
        const supervisorName = remit.supervisor_receiver 
          ? `${remit.supervisor_receiver.first_name} ${remit.supervisor_receiver.last_name}`
          : "Supervisor";
        const ticketerName = remit.ticketer 
          ? `${remit.ticketer.first_name} ${remit.ticketer.last_name}`
          : "Ticketer";
        await sendNotification({
          companyId: companyId,
          message: `System issued a late bank deposit fine of ₦${formattedAmount} to ${supervisorName} for remittance from ${ticketerName}.`,
          type: "FINE_ISSUED",
          referenceId: fine.id,
          target: {
